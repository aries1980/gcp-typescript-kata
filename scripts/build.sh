...
build() {
  ...
  zip -r ./gcp-typescript-kata-birthday.zip ./dist/*
  ...
  docker run \
    ...
    google/cloud-sdk \
      bash -x -e -c "\
        ...
        gsutil cp gcp-typescript-kata-birthday.zip gs://gcp-typescript-kata-function-storage/$PACKAGE_VERSION-gcp-typescript-kata-birthday.zip \
      "
}
